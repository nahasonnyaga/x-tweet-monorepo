...
// Define motion variants locally
const viewTweetVariants: Variants = {
  initial: { opacity: 0 },
...
export function ViewTweet(tweet: ViewTweetProps): JSX.Element {
  ...
  return (
    <motion.article
      className={cn(
        `accent-tab h- relative flex cursor-default flex-col gap-3 border-b border-light-border px-4 py-3 outline-none dark:border-dark-border`,
        reply && 'scroll-m-[3.25rem] pt-0'
      )}
      {...viewTweetVariants}
      animate={{ ...viewTweetVariants.animate, transition: { duration: 0.2 } }}
      exit={undefined}
      ref={viewTweetRef}
    >
      <Modal
      ...
      {reply && (
        <p className="text-light-secondary dark:text-dark-secondary">
          Replying to{' '}
          <Link href={`/user/${parentUsername}`}  className="custom-underline text-main-accent">@{parentUsername}</Link>
        </p>
      )}
      ...
      <Input reply parent={{ id: tweetId, username }} />
    </motion.article>
  );
}
