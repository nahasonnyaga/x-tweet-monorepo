...
export function Tweet(tweet: TweetProps): JSX.Element {
  ...
  // ✅ Safe includes checks
  const tweetIsRetweeted = Array.isArray(userRetweets) ? userRetweets.includes(profileId ?? '') : false;
  const hasLiked = Array.isArray(userLikes) ? userLikes.includes(userId) : false;
  const hasReplied = Array.isArray(userReplies) ? userReplies.includes(userId) : false;

  // ✅ Extract hashtags
  const hashtags = text?.match(/#\w+/g)?.map(tag => tag.slice(1).toLowerCase()) ?? [];

  // ✅ Update Firestore (once per mount)
  if (hashtags.length && tweetId) {
    (async () => {
      try {
        const tweetRef = doc(db, 'tweets', tweetId);
        await updateDoc(tweetRef, { hashtags });
      } catch (err) {
  ...
  // ✅ Render tweet text with clickable hashtags
    ...
    >
      ...
      <Link
        ...
        className={cn(
          ...
          parentTweet ? 'mt-0.5 pt-2.5 pb-0' : 'border-b border-light-border dark:border-dark-border'
        )}
        draggable={false}
      ...
      >
        <div className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1">
            ...
            {modal ? null : pinned ? (
              ...
              tweetIsRetweeted && (
                <TweetStatus type="tweet">
                  <Link
                    href={`/user/${profileUsername as string}`}
          ...
          <div className="flex min-w-0 flex-col">
            ...
            {text && <p className="whitespace-pre-line break-words">{renderTweetText(text)}</p>}
