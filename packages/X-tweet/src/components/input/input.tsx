...
import { addDoc, serverTimestamp, WithFieldValue } from 'firebase/firestore';
import { tweetsCollection } from '@lib/firebase/collections';
import { manageReply, manageTotalTweets, manageTotalPhotos } from '@lib/firebase/utils';
...
export function Input({
  ...
  const sendTweet = async (): Promise<void> => {
    ...
    const hashtags = inputValue
      ? Array.from(new Set(inputValue.match(/#\w+/g)?.map(tag => tag.slice(1).toLowerCase()) || []))
      : [];
    ...
    await Promise.all([
      ...
      uploadedUrls.length && manageTotalPhotos('increment', userId),
      isReplying && parent?.id && manageReply('increment', parent.id),
    ]);
    ...
    toast.success(
      () => (
        <span className="flex gap-2">
          Your Tweet was sent
          <Link href={`/tweet/${tweetRef.id}`} legacyBehavior  className="custom-underline font-bold">View</Link>
        </span>
      ),
      { duration: 6000 }
    );
  ...
  const removeImage = (targetId: string) => (): void => {
    ...
    const found = imagesPreview.find(({ id }) => id === targetId);
  ...
  const handleChange = ({ target: { value } }: ChangeEvent<HTMLTextAreaElement>): void =>
    setInputValue(value);
  ...
  return (
    ...
    >
      ...
      {reply && visited && (
        ...
        >
          Replying to{' '}
          <Link href={`/user/${parent?.username as string}`} legacyBehavior  className="custom-underline text-main-accent">{parent?.username as string}</Link>
        </motion.p>
      )}
      ...
      <label
        className={cn(
          ...
          reply ? 'pt-3 pb-1' : replyModal ? 'pt-0' : 'border-b-2 border-light-border dark:border-dark-border',
